...
public class ViewAtController {
	...
	@RequestMapping(value="/message/at/view/{no}", method=RequestMethod.GET)
	public String inform(@PathVariable int no, Model model, 
		...
		Page<Message> messages = messageRepository.findByToAndtype(user.getId(), MessageType.AT.name(), pageable);
		int size = messageRepository.findByToAndtype(user.getId(), MessageType.AT.name()).size();
		int total = 0 ;
		...
		return "message/at.single";
	}
	
	@RequestMapping(value="/message/at/{id}/set", method=RequestMethod.GET)
