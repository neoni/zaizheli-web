	...
	<c:forEach var="message" varStatus="stat" items="${messages}">
		<dl class="mes-list row-fluid">
			...
			<dd class="content" style="margin-left: 65px;margin-top: 6px;">	
				...
				<p class="">
					<span style="margin-right:80px;">来自 <a href="<c:url value="/profiles/${message.from.id}"/>">${message.from.name}:</a></span>
					<a style="color:#000000;" href="<c:url value="/message/pm/${message.id}/detail" />"><b>${message.title}</b></a>	
				</p>
				</c:when>
				<c:otherwise>
				<p class="">
...
<script type="text/javascript">
	var options = {
		...
		pageUrl: function(type, page, current){
                return '<c:url value="/message/pm/to/" />' + page ;
            }		
		}
	$('#m_page').bootstrapPaginator(options);	
</script>
