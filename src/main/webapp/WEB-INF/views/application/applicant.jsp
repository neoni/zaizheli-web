...
<div class="thumbnail pin wft" style="-moz-transition: left 0.5s ease-in-out 0s, top 0.5s ease-in-out 0s; margin:6px;">
	...
	<div class="convo row-fluid">
		<c:if test="${application.activity.apply == 1}">
			<h4><a href="<c:url value="/application/${application.id}/view" />" style="text-align:center">查看该成员的申请表</a></h4>
		</c:if>
		<c:if test="${application.status eq '申请中'}">
		<div style="float:right; margin-top:8px;">			    
			<a  class=" btn btn-danger b" act="<c:url value="/application/${application.id}/agree" />" onclick="op.apply_agree(event.currentTarget); event.preventDefault();">
					同意</a>
			<a  class="btn btn-info ml-10" act="<c:url value="/application/${application.id}/refuse" />" onclick="op.apply_refuse(event.currentTarget); event.preventDefault();">拒绝</a>
		</div>	
		</c:if>
		<c:if test="${application.status eq '已加入'}">
		<div style="float:right; margin-top:8px;">			    
			<a  class="btn btn-inverse ml-10" act="<c:url value="/application/${application.id}/kickout" />" onclick="op.apply_kickout(event.currentTarget); event.preventDefault();">踢出此活动</a>
		</div>	
		</c:if>
	</div>
</div>
